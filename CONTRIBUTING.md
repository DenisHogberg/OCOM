...
Every push and pull request runs the checks in `.github/workflows/ci.yml`: a required-documents check, markdownlint over every Markdown file, lychee over every link, a publication-metadata job that keeps the Publication Manifest, the Specification version fields, the Status fields and every document's revision history consistent with each other, a requirement-register job that regenerates the Requirement Register from the documents Chapters 4 to 6 compile and fails if the committed file differs or an alias is missing or bound twice, a principle-traceability job that re-verifies every citation in `docs/Governance/Principle-Traceability.md` against the corpus, a tool-tests job that runs the negative tests in `tools/tests/`, and reuse lint over the licensing map. Two more arrive from their own workflows: the dependency review in `.github/workflows/dependency-review.yml`, which runs on pull requests and fails when a change introduces a dependency with a known vulnerability of high or critical severity, and the sign-off check. The branch ruleset on `main` lists the required contexts; `gh api repos/DenisHogberg/OCOM/rulesets/23337033` prints the current list, which is the only place worth trusting for a count. `main` takes no direct pushes and the ruleset has no bypass list, so a failing check blocks every merge, including the maintainer's. [CI-DESIGN.md](CI-DESIGN.md) explains why each check exists and how to run the same checks locally.
