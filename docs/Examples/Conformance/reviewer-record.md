...
**Recorded against:** model `066a4ba34e504aee`, map `b424bff6d7fc4f0e`, statement `cf478add0bddcc36` (the first sixteen hex digits of each file's SHA-256, as `tools/conformance/validate.py` prints them). The tool refuses this record against any other export: a judgment recorded against one model is a judgment about that model.
