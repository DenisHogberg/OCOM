...
| GAP-007 | Ten tracked documents carry no Revision History table and eight of them no Last Updated field, against this register's own standard (`Documentation-Standards.md`): the five `Adoption/` documents, `Examples/Implementation-Case/Performance-Marketing-Operator-RU.md`, `Reference Architecture/Overview.md`, `Specification/Committee Review Package.md`, and the two Conformance Test Suite artifacts `Governance/Requirement-Register.md` and `Governance/Requirement-Aliases.md`. Recorded 16 September 2026 as sixteen documents, which counted the nine `Specification/` chapters, since closed, and missed the Russian implementation case; the two suite artifacts are covered by the generated-and-append-only convention `Documentation-Standards.md` records on 17 September 2026 and are listed here for completeness rather than as debt. | Low | `Adoption/*.md`, `Examples/Implementation-Case/Performance-Marketing-Operator-RU.md`, `Reference Architecture/Overview.md`, `Specification/Committee Review Package.md` | Either add the table and field to each, or amend the standard to name the Compiled tier's Source-line convention as the equivalent; decide once, not per document | Open; not required for v1.0, recount and correction 17 September 2026 |
