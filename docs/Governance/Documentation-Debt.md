...
| GAP-007 | Sixteen tracked documents carry no Revision History table, and fifteen of them no Last Updated field (`Reference Architecture/Overview.md` carries one), against this register's own standard (`Documentation-Standards.md`: Revision History mandatory, every document): the nine `Specification/` chapters and `Specification/Committee Review Package.md`, which record edits in their Source lines instead, the five `Adoption/` documents, and `Reference Architecture/Overview.md`. `PROJECT_STATUS.md` was the seventeenth and gained its table on 16 September 2026. | Low | `Specification/*.md`, `Adoption/*.md`, `Reference Architecture/Overview.md` | Either add the table and field to each, or amend the standard to name the Compiled tier's Source-line convention as the equivalent; decide once, not per document | Open; not required for v1.0, recorded by the dogfooding audit of Release v1.2.0 |
