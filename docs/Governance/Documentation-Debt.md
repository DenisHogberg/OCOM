...
| FW-010 | `Models/Domain.md` and `Models/Entity.md` bind an Entity to "exactly one primary Domain", while `Entities/Overview.md`'s Conformance clause and fourteen Entity documents require a "governing Domain"; the two names denote one relation, no document says so, and no tier above Entity defines the second name | Undetermined | `Models/Domain.md`, `Entities/Overview.md`, `docs/Entities/` | CDKO deep logic check, 18 September 2026 | Open; which name governs is a decision rather than an editorial fix |
...
| 0.1 | 21 September 2026 | `FW-001` closed: `CAND-023` decided and integrated, the four sections of `Memory/Evidence Overlay.md` are written. |
