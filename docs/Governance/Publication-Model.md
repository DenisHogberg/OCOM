...
| **Release** (public, citable identifier) | GitHub tag/release, resolved by `Publication-Manifest.md` | `v1.3.0` (current, 17 September 2026; `v1.2.0` and `v1.1.1` Superseded; `v1.0.0` was mislabeled, see below) | An independent semver track. **Does not mirror any single component's version number.** Names one specific bundle: {this Constitution version + this Core Vocabulary version + this Specification version + this commit}. |
