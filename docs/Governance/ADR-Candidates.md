...
**Postscript (22 September 2026, from `AO-085`):** clause 3 says what an erasure record carries and said nothing about who may issue one, so the one path that removes a record from verification was open to any writer an implementation admits. The Chief Architect adopted the smallest closure as a postscript rather than a new candidate: `Memory/Retention.md`'s Deleted state now requires an erasure record to name a Policy the organization has declared and the actor who issued it, and an implementation to refuse one that names neither. `tools/conformance/validate.py` grants the Integrity exclusion only to an erasure record whose Policy the export declares and whose actor is named, verifies the records the other erasure records name like any other, and reports every erasure record in an Erasure Records section of the report; `Memory/Retention.md` is outside the twenty-two-document requirement set, so this is an observation the report carries and not a catalogued Test. The three clauses are unchanged.
