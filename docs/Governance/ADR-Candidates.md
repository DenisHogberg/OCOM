...
**Postscript (19 September 2026):** Step 2 items done under the authorization given the same day: the EPIC-F scope note, and the rule sentences in `Governance/Publication-Model.md`'s Projection tier. What remains is publication work rather than repository work: renaming the two local types in `graph.jsonld` so they expand to something the publisher defines, publishing those definitions, and the row in `tools/site/publication_health.py` that fails when a coined name expands to nothing. `AO-078` stays Open in part until that is done and the tool reports it against the live site.
