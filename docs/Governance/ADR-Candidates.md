...
**Postscript (22 September 2026):** questions 1 and 2 have a worked form. `CAND-027` expresses inter-organizational structure with the existing primitives question 1 named, Composition Relationships between Organizations and legal form as a Classification, and internal structure as the Department Entity, and `CAND-025` reads the single-owner rule question 2 rests on as accountability singular and responsibility plural. Questions 4, 5 and 7, and the parts of 2, 3 and 6 that need a Reference Case, stand as above.
...
**Next Action:** Two-step discipline. Step 1, the Decision, is recorded above. Step 2, authorized with it and executed on 21 September 2026: the four sections written into `Memory/Evidence Overlay.md` with a Revision History row citing this candidate; `FW-001` closed in `Documentation-Debt.md`; `Test-Catalogue.md` regenerated, since the Integrity kind reassigns existing Statements (recorded on 21 September 2026 as "`Requirement-Register.md` and `Test-Catalogue.md` regenerated, since the new Conformance clause adds Statements"; the register is unchanged and cannot change here, because `Memory/Evidence Overlay.md` is outside the twenty-two-document requirement set, and round 5 of the all-packages test read the claim against the commit); `AO-084` narrowed. Added 24 September 2026, after round 7 of the all-packages test read the corpus against this entry: `Core/Terminology.md` carries Evidence as an entry of the index since 23 September 2026, with two Revision History rows citing this candidate, and no item here named it. It is an index entry quoting a Definition this Decision wrote, which is the documentation-currency change `CAND-007` Section 3 permits and the form `EPIC-D` set for that document; recording it here is what was missing.
...
**Postscript (22 September 2026, from `AO-085`):** clause 3 says what an erasure record carries and said nothing about who may issue one, so the one path that removes a record from verification was open to any writer an implementation admits. The Chief Architect adopted the smallest closure as a postscript rather than a new candidate: `Memory/Retention.md`'s Deleted state now requires an erasure record to name a Policy the organization has declared and the actor who issued it, and an implementation to refuse one that does not name both (restated from "names neither" on 23 September 2026, with `Memory/Retention.md` and `AO-085`; round 8 of the all-packages test found this postscript still carrying the weaker wording). `tools/conformance/validate.py` grants the Integrity exclusion only to an erasure record whose Policy the export declares and whose actor is named, verifies the records the other erasure records name like any other, and reports every erasure record in an Erasure Records section of the report; `Memory/Retention.md` is outside the twenty-two-document requirement set, so this is an observation the report carries and not a catalogued Test. The three clauses are unchanged.
...
**Status:** ✅ Decided, 22 September 2026, adopted as drafted by the Chief Architect, the same day it was filed under `EPIC-B` (Object Model consolidation, the route `CAND-016` took for Relationship). Integration was authorized together with the Decision, on the form `CAND-022` set, and executed on 22 September 2026; the Next Action below records what was done.
...
**Next Action:** Two-step discipline. Step 1, the Decision, is recorded above. Step 2, authorized with it and executed on 22 September 2026: the sentence in `Models/Entity.md` Ownership and the sentence in `Meta/Ownership.md` Shared Ownership; `tools/conformance/validate.py`'s Presence procedure counts one where a Statement asks for one, so `REQ-MODELS-ENTITY-004` now fails on an Entity with two owner values, on an owner that names an Ownership record for another Object, and on an Entity that several Ownership records name while its owner says which is accountable to none of them; `Conformance-Test-Suite.md` Section 3 says so; `Adoption/Getting Started.md` Section 4 and `First Pilot.md` step 3 say what Owner means; `AO-015` is Closed in part, its modality half standing. The four Statements the catalogue's cardinality rule holds at Review (`REQ-MODELS-ENTITY-007`, `REQ-MODELS-EVENT-009`, `REQ-MODELS-LIFECYCLE-001`, `REQ-LIFECYCLES-001`) are not moved by this Decision; a Decision that moves them would name them. (Recorded as eleven when this entry was written; the recount of 22 September 2026 found four, and five further Review rows that carry a cardinality phrase but leave for an earlier rule.) Corrected on 23 September 2026, after round 3 of the all-packages test: the leg counted nothing when the Representation Map listed no Ownership collection, and took the record the Entity's owner named by identifier without looking at whether a second Ownership record named the same Entity, so "exactly one Ownership record of the accountable Type per Entity" was enforced by nothing wherever the owner named a record rather than a party; and no part of the tooling read an Ownership Type at all. The map now declares which Ownership Type the organization holds accountable, in a row `Ownership.accountable type` of kind declaration beside a field row `Ownership.type`, and the Test counts the records of that Type; where the map declares neither, the Test says so in its own reason rather than reporting the Decision's rule.
...
**Status:** ✅ Decided, 22 September 2026, adopted as drafted by the Chief Architect, the same day it was filed as a publication decision on the form `CAND-012` set for Adoption pages. Integration was authorized together with the Decision, on the form `CAND-022` set, and executed on 22 September 2026; the Next Action below records what was done.
...
**Next Action:** Two-step discipline. Step 1, the Decision, is recorded above. Step 2, authorized with it and executed on 22 September 2026: `docs/Adoption/Reference Serialization.md`, Informative, published at `ocom.uno/adoption/reference-serialization` on `CAND-012`'s terms; `docs/Examples/Conformance/schema.json`, generated by `tools/conformance/reference_schema.py` from `model.json` and checked against a regeneration and against the model in CI; the map row `Identity.scope` of kind `declaration` in `docs/Examples/Conformance/representation-map.md`; `REQ-META-IDENTITY-005` bound to a Declaration Test that reads the declared scope, passes one of the five, fails any other or an External System that names no system, and is pending when the map declares none; the site's `adoption` index, `sitemap.xml`, `discovery.json` and `llms.txt` name the page.

**Postscript (22 September 2026, from the acceptance evaluation):** the six-lens evaluation rerun the day of integration ran the suite on an export shaped like an enterprise's, SAP business-partner keys beside ServiceNow keys, and found two things the Decision's text had promised and the tools had not delivered. The suite compared identities as bare strings, so `1000001` from two declared systems failed `REQ-META-OBJECT-003` and `REQ-META-IDENTITY-008` while the page said they coexist; and the derived schema required every collection of the example, so a minimal export failed twenty checks before any OCOM rule was read. Both fixed the same day: `tools/conformance/validate.py` keys identity uniqueness and reuse by the declared scope and system, declared per type or per collection path, and reports a bare reference to an identity present in two scopes as ambiguous; `tools/conformance/reference_schema.py` makes collections optional and requires a property only where at least two example records carry it. Three negative tests cover the cases. The Decision is unchanged; what it does not do stands: scope semantics (`AO-009`) and merge and split (`AO-010`) are still open, and a reference cannot yet say which of two scopes it means.
...
**Status:** ✅ Decided, 22 September 2026, adopted as drafted by the Chief Architect, the same day it was filed under `EPIC-B` and `EPIC-C` (Object Model consolidation; Domain and Entity integration). Integration was authorized together with the Decision, on the form `CAND-022` set, and executed on 22 September 2026; the Next Action below records what was done.
...
**Next Action:** Two-step discipline. Step 1, the Decision, is recorded above. Step 2, authorized with it and executed on 22 September 2026: `Entities/Department/Department.md` written to the structure `Entities/Overview.md` requires, replacing the stub; one paragraph in `Meta/Organization.md` after the sentence that deferred inter-organizational modeling to governance, carrying no keyword and therefore no new Statement, so the register and the catalogue are unchanged by it; one paragraph in `Adoption/First Pilot.md`; `CAND-004` postscript for questions 1 and 2.
...
1. It answers the operator's question with what the corpus already holds. Every responsibility it names is a rule the specification states (`Models/Domain.md`, `Models/Entity.md`, `Conformance-Test-Suite.md`); every artifact is one the tooling produces or reads (the seven-field definition, the export, the map, the Test Report, the Reviewer Record); the order is `First Pilot.md`'s seven steps.
2. It keeps the discipline on numbers. The corpus states no effort figure because it has no evidence for one, and the Evidence Register says what evidence exists; the plan turns that absence into an arithmetic the adopter fills in and then measures, which is the only honest form until a pilot reports.
3. It is the form `CAND-010` and `CAND-011` set: an Informative document, restating, authorized by its own Decision, published only after it.
4. It moves the lowest lens without moving a rule: the evaluation's first finding is closed by the existence of the plan, and its "what would change my mind" names this document first.

### Scope and the Architecture Freeze

Filed under `EPIC-F` on the form `CAND-010` used; `CAND-007` Section 3 permits editorial and adoption work and Section 4 holds: no concept, no Canonical Principle, no Entity type, no Conformance Profile, nothing normative. The document's Draft status until the Decision keeps the repository's own rule that an Adoption page qualifies only once it has its own Decision.
