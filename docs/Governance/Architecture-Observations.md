...
**Status:** Closed in part, 21 September 2026. The Retention half is closed: `CAND-024` restated `Memory/Retention.md`'s Deleted state as an erasure that preserves the record's identity, creation time, creator and demonstration of integrity and is itself recorded as a new Memory Record, so it no longer stands against Constitution Principles 4 and 6. The `Memory/Layered Memory.md` half, its demotion text against the same Principles, stays open under the EPIC-A disposition of 18 September 2026 and moves through a Reference Case of its own.
...
**Description:** Surfaced by a deep test of the repository and the site (18 September 2026). The traceability check run on 17 September 2026 for the v1.0 recompilation, and the discipline the chapters record in their Revision Histories, both run in one direction: every sentence of a chapter is traced to a sentence of a source document. Nothing runs the other way, from each mandatory Statement of a source document to the chapter that compiles it. Three obligations were therefore missing from `Specification/05 Object Model.md` while every sentence it carried traced correctly: the ninth item of `Models/Entity.md`'s "Every Entity shall" list ("be governed by the rules of this specification"), the fifth of its prohibitions ("have ambiguous meaning"), and all four prohibitions of `Models/Relationship.md:122-128`. They are compiled into the chapter by the change that records this entry. A fourth instance was found on 19 September 2026 while preparing `CAND-018` and is corrected by the change that files it: `Specification/02 Design Principles.md`, whose Source line declares verbatim compilation, did not carry `Core/Principles.md`'s subordination sentence, "These Principles operate within, and shall not contradict, the Canonical Principles of the OCOM Constitution". `Governance/Requirement-Register.md` enumerates the Statements of the 22 canonical documents, so the raw material for a backward check exists; nothing consumes it that way. *(19 September 2026: the backward check now exists. `tools/conformance/compilation_survey.py` reads each chapter's Source line, extracts the mandatory Statements of the documents it names with the same parser the Requirement Register uses, and reports for each Statement whether the chapter carries it verbatim, carries it in paraphrase, or does not carry it. `CAND-018` makes only the third Source-line form a completeness claim, so the tool enforces that form and counts the rest; it runs as a required CI job. The first survey: the two chapters declaring verbatim carry every mandatory Statement of the documents they name, 2 of 2 for `01 Introduction.md` and 10 of 10 for `02 Design Principles.md`, which is the calibration, since both were checked by hand on the same day. The five declaring plain `compiled from` carry 28 of the 249 mandatory Statements their sources hold: 3 of 107 in `04 Meta Model.md`, which says in its own Source line that it is a normative summary, 8 of 62 in `05 Object Model.md`, 9 of 51 in `06 Lifecycle Model.md`, 1 of 8 in `07 Governance.md`, and 7 of 21 in `08 Conformance.md`. The two declaring `synthesized from` carry none, as that form declares. Every one of those omissions is permitted by `CAND-018` and disclosed by the form the chapter declares; what the numbers add is how much of the normative text a reader of the reading path never meets, which is largest exactly where a reader is least likely to expect it: seven of the eight Governance Principles that govern the specification itself, including "Every architectural concept shall be traceable across the specification" and "Every version of the specification shall be reproducible", appear in no form in the Governance chapter. One enumeration defect found by the same survey is recorded separately as `AO-082`.)*
...
**Status:** Closed, 19 September 2026. Both halves of the entry's condition are met: `CAND-018` records where the obligation sits, a chapter may abridge and an omission from it is a defect of the reading path rather than a discharge, and the backward check now exists as `tools/conformance/compilation_survey.py`, enforced for the one form that claims completeness and reported as a census for the rest. The four instances the entry names are compiled into their chapters.

**Architect Response:** Chief Architect, 19 September 2026, recorded in `CAND-018`, and completed later the same day: the backward check is built and required, and it enforces the verbatim form only, because that is the only form `CAND-018` makes a completeness claim. A chapter that abridges is measured, not failed.
...
**Status:** Open in part. `CAND-020` (Decided 19 September 2026) settles which Statement a claimant is measured against, so the published conformance level is attainable again. The contradiction itself stands: two mandatory sentences of two Draft documents still say opposite things about the same construct, which is `AO-028`'s question and still waits for a Reference Case.

**Architect Response:** Chief Architect, 19 September 2026, recorded in `CAND-020`. A claimant is measured against `REQ-MODELS-LIFECYCLE-002`; `REQ-LIFECYCLES-004` carries the Disposition Descriptive and its Test reports Not Applicable. No canonical document changed.
...
**Status:** Open in part. `CAND-021` (Decided 19 September 2026) records that the canonical definition is `Language/Conformance.md`'s and that the chapter originates nothing, which removes the contradiction with `CAND-018`. What stays open is whether the twenty-two-document scoping becomes canonical text, deferred behind the same Reference Case `CAND-017` waits on.

**Architect Response:** Chief Architect, 19 September 2026, recorded in `CAND-021`.
...
**Title:** Two Canonical Documents Enumerate What Every Entity Shall Do, With Different Items, and Neither Names the Other

**Date observed:** 19 September 2026

**Description:** Surfaced by the backward survey of the compiled reading path (19 September 2026), run to answer the half of `AO-077` that `CAND-018` left open. `Models/Model.md:65` opens "Every Entity shall:" and lists six items: belong to a Domain; define a Lifecycle; occupy one State; participate in Relationships; be affected only by valid Workflows; generate or receive Events where applicable. `Models/Entity.md:41` opens with the same four words and lists nine: possess a unique identity; have operational meaning; belong to exactly one primary Domain; have defined ownership; contain attributes; define one or more states; define a lifecycle; participate in relationships; be governed by the rules of this specification. Neither list contains the other. Two of `Models/Model.md`'s items, "be affected only by valid Workflows" and "generate or receive Events where applicable", appear in no other enumeration in the corpus and in no chapter of the reading path; `Specification/05 Object Model.md` names both documents in its Source line and carries `Models/Entity.md`'s nine items only. `Governance/Requirement-Register.md` holds both lists as separate mandatory Statements, so the enumerated requirement set asks for fifteen obligations under one opening sentence that a reader meets twice in two documents.

**Impact:** An implementer who reads either document alone reads a list whose opening words claim completeness, and is measured against a register that carries both. The two items only `Models/Model.md` states are the ones most likely to be missed: they bind an Entity to valid Workflows and to Events, they reach no reader of the published reading path, and nothing in `Models/Entity.md` hints that a second list exists. This is the condition `CAND-016` resolved for Relationship, where `Meta/Relationship.md` was made canonical and `Models/Relationship.md` its specialization, left unresolved for the pair one tier down.

**Recommendation:** Record only. A Reference Case drawn from a real implementation measured against one list and audited against the other would settle which document owns the enumeration. Two resolutions are available without inventing anything: `Models/Model.md`'s list becomes a cross-reference to `Models/Entity.md`, in the shape `CAND-016` used, and the two items it alone carries move into `Models/Entity.md`; or the two lists are declared to state different things, the model's structural obligations and the Entity's own, and each says so in its own words.

**Status:** Open; not escalated (single internal source; awaiting a Reference Case per Standard Evolution Methodology Rules 1 and 2). It moves when an implementation is measured against one list and audited against the other, or when the Chief Architect records which document owns the enumeration.

**Architect Response:** *(pending)*

**Related:** `Models/Model.md`, `Models/Entity.md`, `Specification/05 Object Model.md`, `Governance/Requirement-Register.md`, `CAND-016`, `CAND-018`, `AO-077`, `AO-046`

---

## AO-083

**Title:** The Site Serves Machine-Facing Files That Nothing in This Repository Sources, and One of Them Published a URL That Does Not Resolve

**Date observed:** 19 September 2026

**Description:** Surfaced by a site verification run on 19 September 2026. `ocom.uno/llms.txt` states the specification's identity, status, licences, author, concept DOI and entry points, and is the file a machine reader is invited to read first. It is written by hand on the server. No document, tool or generator in this repository said what it should contain, so nothing could be reviewed against it and a defect in it could be found only by reading the live file. One was: a sentence ended `Origin story and motivation: https://ocom.uno/why.`, and every harvester that takes URLs from that file asked the site for `/why.`, which answers 404. The same line glued a sentence's semicolon to the concept DOI and a period to the `CITATION.cff` URL. `Publication-Manifest.md` already listed `llms.txt` among the machine-readable projections tied to a release, which made the absence of a source easy to miss: the Manifest names the file as an output of the Publication Engine, and the Engine is gone (`Publication-Model.md`, Known Gaps; `FW-006`).

**Impact:** A published file with no source is a claim no reader can check against a reviewed text, which is the failure `AO-064` names for derived artifacts. The concrete cost here was a broken machine-facing URL that survived until a crawl found it, and a correction that, until this change, would have left nothing behind in the repository: the next person to edit the server copy would have had no way to know what the file was supposed to say.
...
**Status:** Closed, 21 September 2026. Both candidates it escalated are decided and integrated: `CAND-023` wrote the four reserved sections of `Memory/Evidence Overlay.md`, and `CAND-024` placed the integrity guarantee in the Memory tier, defined Audit Record by reference and restated erasure. `Conformance-Test-Suite.md` Section 3 carries the Integrity kind, which can return Fail, and `tools/conformance/validate.py` implements it against a declared demonstration and passes only content-addressed identity, the one named mechanism that demonstrates to a party holding nothing but the record and its identity; the example's Audit Records and Events carry such identities, its Evidence Records are contained but reached by no Test. The first boundary condition, the publication's own records carrying no demonstration, is narrowed rather than closed: the example export now carries one, the site's records do not yet.
...
**Impact:** An implementation can change, be re-exported, and keep the Review Passes recorded against what it used to be. The report names the digests of the run's own inputs, so a reader who compares two reports can see that the export changed; nobody is required to look, and the suite does not. **As of 23 September 2026 this is no longer the state of the tooling:** the closure the Recommendation names shipped on 22 September (the Reviewer Record's `**Recorded against:**` line and the validator's refusal of a record recorded against another export), and round 3 of the all-packages test closed the escape that remained, a record declaring one of the three digests and binding its judgments to that alone. The figures in the Description are those of 22 September; after the Declaration Tests on extensions were corrected on 23 September the run reads 135 of 185. The entry stays Open because its Status is the Chief Architect's to move, not the tooling's.
