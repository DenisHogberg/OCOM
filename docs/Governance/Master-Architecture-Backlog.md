...
**Execution continued (16 September 2026).** Under the Chief Architect's instruction to bring the specification to the Part 8 bar without waiting for implementation cases, EPIC-D, EPIC-C, EPIC-B and EPIC-F are executed and `CAND-002` is decided and integrated; see the execution notes under each Epic. Part 8 items still open: EPIC-A (Layer 2 gated; disposition pending) and EPIC-E's Test Suite specification. The documents changed by these executions carry the change in their own revision histories.
