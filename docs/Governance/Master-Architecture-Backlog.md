...
**Disposition extended (18 September 2026):** the Chief Architect extends the 16 September 2026 disposition to the two documents named above. `Memory/Layered Memory.md` and `Memory/Retention.md` do not block the v1.0 claim, and this Epic stays closed for it. The reasoning is recorded in full under `AO-069`: the tension is disclosed rather than concealed, in this entry, in the register and in a dated editorial note inside each of the two documents; its correction is a Memory-model change that `CAND-007` Section 4 places behind Section 5 and that no Reference Case yet supports; and the v1.0 bar of Part 8 is a documentation bar under which 61 other observations are open. The contradiction stays open as `AO-069`, and the next Release body and `Release-Readiness.md` disclose it in the same words.
...
**Execution note (16 September 2026):** `AO-001` was decided on 11 September 2026 (`CAND-015`) and its Step 2 integration is now performed: `Domains/Common/Domain.md` is Informative and names `Models/Domain.md` as canonical. `AO-002` is Closed by `CAND-016`: `Meta/Relationship.md` is the canonical definition of Relationship with Objects as participants and `Models/Relationship.md` its specialization for Relationships between Entities, so the two no longer disagree; one sentence in `Models/Relationship.md`'s Purpose and the Terminology entry carry it. `CAND-004`'s seven questions each carry a disposition recorded in its postscript: questions 1, 2 (single owner), 3 (mechanism) and 6 (Identity scope) are resolved by existing primitives; questions 2 (several owners), 3 (the recorded omission), 4, 5, 6 (Registry scope) and 7 are not required for v1.0 and need Reference Cases under `CAND-007`. Definition of Done met on the terms Part 8 states.
...
**Execution note (16 September 2026):** `CAND-002` is Decided: Profile Conformance is defined by the form of a claimant's declaration, per `Governance/Concept-Paper-Profile-Conformance.md`. Its integration into `Specification/08 Conformance.md`, `Language/Conformance.md` and `Adoption/FAQ.md` was performed on 16 September 2026 under EPIC-F; the declaration Projection and the CI validator remain open as tooling. The Conformance Test Suite specification exists since 16 September 2026 (`Governance/Conformance-Test-Suite.md`): the requirement set computed from Chapters 4 to 6, a Requirement Register derived by rule and never edited (335 Statements at that commit, answering `AO-062` in part), five Test kinds, the Test Report, and the tooling that remains, which Part 8 does not require; on 17 September 2026 the register generator, the Alias File (335 aliases) and the CI job that regenerates the register and checks alias coverage were added under `tools/conformance/`, leaving the Representation Map validator and the first report. The guarantees it tests are the twenty two documents of that set, none of which is the World Model document `CAND-014` leaves to Layer 2; that is how the Part 5 and Part 6 dependency on EPIC-A is met, by the disposition of the same day rather than by Layer 2. Definition of Done met on the terms Part 8 states.
...
**Execution note (16 September 2026):** executed under `CAND-007` Section 3 as EPIC-F. `Specification/` is neither retired nor superseded: it is the compiled reading path per `Publication-Model.md`, and it now reflects the Constitution where a reader needs it, a Relationship to the Constitution section in Chapter 1, a Constitution and Architecture Freeze section in Chapter 7, and one sentence in Chapter 0. Chapter 4 gains an Organization section compiled from `Meta/Organization.md` (`AO-065` Closed). Chapter 8's Note on scope now points to the `CAND-002` grounding paper, as do `Language/Conformance.md` and `Adoption/FAQ.md`. `Adoption/README.md` and `docs/README.md` name `Core/Constitution.md` as the governing document and the reading path as a compilation; `PROJECT_STATUS.md` carries a currency note. `Examples/Overview.md` lists the two collections that exist. `DEBT-DOC-001`, `GAP-001`, `FW-002` and `FW-005` each carry a not-required-for-v1.0 disposition in `Documentation-Debt.md`. `Specification/Committee Review Package.md` carries a postscript on `CAND-001` and `CAND-002`. The Worked Example under `Adoption/` exists per `CAND-010`. Definition of Done met on the terms Part 8 states.
...
| ✅ Design Conformance Test Suite. Specified 16 September 2026, `Governance/Conformance-Test-Suite.md`; tooling remains open, not required for v1.0 | E | Critical | XL | High — largest single deliverable in the backlog | EPIC-A, EPIC-D (guarantees must be stable first) |
...
| ✅ Refresh or retire `Specification/` v0.2; re-point `Adoption/`. Done 16 September 2026: reading path kept and re-pointed to the Constitution (Chapters 0, 1, 7), `Adoption/` and `docs/README.md` re-pointed | F | High | M | Low | Ideally after EPIC-D (so it reflects the closed §9/§11 wording too), but not blocked by it |
| ✅ Align `Examples/Overview.md` scope with reality. Done 16 September 2026 | F | Low | S | Low | None |
| Close `DEBT-DOC-001`, `GAP-001`, `FW-002`, `FW-005`. Dispositioned 16 September 2026 as not required for v1.0 | F | Low | S | Low | None |
...
| 0.1 | 22 September 2026 | `CAND-025` and `CAND-027` (EPIC-B, EPIC-C) and `CAND-026` (publication) Decided and integrated the day they were filed; `Entities/Department/` written under EPIC-C, closing the dangling primary Department of Employee and Team. |
