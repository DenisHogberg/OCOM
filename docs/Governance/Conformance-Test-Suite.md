...
An implementation is Core Conformant, in the sense of Chapter 8, when every mandatory Test is Pass or Review Pass. One Fail on a mandatory Test is Non-Conformance and Chapter 8's clause applies: the implementation shall not claim conformance with that version, and partial support may be documented without a claim. Recommended and optional outcomes are reported and do not affect the Core claim.
...
`Master-Architecture-Backlog.md` Part 8 requires this specification to be checkable in principle and does not require the tooling. What remains, each item small and each derivable from the rules above: the register generator implementing Section 2; the Alias File and its first population; a CI job that regenerates the register and fails on divergence from any published copy; a validator for the Representation Map; and the first submitted report. None of these edits a normative document.
