...
**C1 against T1.** Direct pushes require the maintainer's SSH key and a GitHub account protected by two-factor authentication. Every commit is attributable in the public history. From the first release after 13 September 2026, release tags are signed with the maintainer's SSH key and can be verified with the key published in `.github/allowed_signers` (SECURITY.md, Verifying releases). Since 14 September 2026 a branch ruleset on `main` enforces the rest: deletion and force pushes are blocked, every change arrives through a pull request, the checks in `.github/workflows/` are required (seven from 14 September 2026, eight from 17 September 2026 when the requirement-register check joined them, and ten from 18 September 2026 with principle traceability and the tool tests; `gh api repos/DenisHogberg/OCOM/rulesets/23337033` prints the list that binds), and the bypass list is empty, so the rule binds the maintainer as well.
...
| 0.5 | 17 September 2026 | C1: the ruleset's required checks are eight since 17 September 2026, the requirement-register check having joined the seven of 14 September 2026; nothing else in the claim changes. |
